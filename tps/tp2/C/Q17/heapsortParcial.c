...
int compareDirectorTitle(const Show* show1, const Show* show2) {
    int cmpDirector = strcasecmp(safe(show1->director), safe(show2->director));
    if (cmpDirector != 0) return cmpDirector;
    return strcasecmp(safe(show1->title), safe(show2->title));
}


void heapify(MyList* l, int n, int i) {
    int maior = i;
    int left = 2 * i + 1;
    int right = 2 * i + 2;

    if (left < n && compareDirectorTitle(l->items[left], l->items[maior]) > 0)
        maior = left;
    if (right < n && compareDirectorTitle(l->items[right], l->items[maior]) > 0)
        maior = right;

    if (maior != i) {
        Show* temp = l->items[i];
        l->items[i] = l->items[maior];
        l->items[maior] = temp;
        heapify(l, n, maior);
    }
}
...
void heapsortPartial(MyList* l, int k) {
    int n = l->n;
    if (k > n) k = n;
    ...
    for (int i = k; i < n; i++) {
        if (compareDirectorTitle(l->items[i], l->items[0]) < 0) {
            Show* temp = l->items[i];
            l->items[i] = l->items[0];
            l->items[0] = temp;
            heapify(l, k, 0);
...
void insertionSortTopK(MyList* l, int k) {
    for (int i = 1; i < k; i++) {
        Show* key = l->items[i];
        int j = i - 1;
        while (j >= 0 && compareDirectorTitle(l->items[j], key) > 0) {
...
int main() {
    ...
    heapsortPartial(choosenShows, 10);
    insertionSortTopK(choosenShows, 10);

    int limit = (choosenShows->n < 10) ? choosenShows->n : 10;
