...
int compareTypeTitle(const Show* show1, const Show* show2) {
    int cmpType = strcasecmp(safe(show1->type), safe(show2->type));
    if (cmpType != 0) return cmpType;
    return strcasecmp(safe(show1->title), safe(show2->title));
}

void insertIntoTop10(MyList* l, Show* key, int k) {
    int j = k;
    while (j > 0 && compareTypeTitle(l->items[j - 1], key) > 0) {
...
void insertionPartial(MyList* l) {
    for (int i = 1; i < l->n; i++) {
        Show* key = l->items[i];

        if (i < 10) {
            insertIntoTop10(l, key, i);
        } else {
            if (compareTypeTitle(key, l->items[9]) < 0) {
                insertIntoTop10(l, key, 9);
            }
        }
    }
}
...
int main() {
    ...
    // Ordena só os 10 primeiros usando inserção parcial conforme o critério novo
    insertionPartial(choosenShows);

    // Imprime só os 10 primeiros
