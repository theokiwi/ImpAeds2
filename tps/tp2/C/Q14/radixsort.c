...
int getMaior();
void radixsort(MyList* array);
void radcountingsort(MyList *array, int exp);
//dateAdded desempate title
void sortByTitle(MyList* array) {
    for (int i = 1; i < array->n; i++) {
        Show* key = array->items[i];
        int j = i - 1;
        while (j >= 0 && compareCase(array->items[j]->title, key->title) > 0) {
...
int getMaior(MyList* array){
    int maior = array->items[0]->release_year;

    for(int i = 1; i < array->n; i++){
        if(maior < array->items[i]->release_year){
            maior = array->items[i]->release_year;
        }
    }
...
void radixsort(MyList* array){
    int max = getMaior(array);
    for(int exp = 1; max/exp > 0; exp *= 10){
        radcountingsort(array, exp);
    }
}
...
void radcountingsort(MyList* array, int exp) {
    ...
    for (int i = 0; i < array->n; i++) {
        count[(array->items[i]->release_year/exp) % 10]++;
    }
    ...
    for (int i = array->n-1; i >= 0; i--) {
        int idx = (array->items[i]->release_year/exp) % 10;
        output[count[idx] - 1] = array->items[i]; // Troca o ponteiro inteiro!
        count[idx]--;
    }

    for (int i = 0; i < array->n; i++) {
        array->items[i] = output[i];
    }
}
...
int main() {
    ...
    sortByTitle(choosenShows);
    radixsort(choosenShows);
