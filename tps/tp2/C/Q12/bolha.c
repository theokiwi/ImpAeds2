...
void bolha(MyList* choosenList);
//dateAdded desempate title
void bolha(MyList* choosenList) {
    for(int i = 0; i < choosenList->n; i++){
        for(int j = 0; j < choosenList->n - 1; j++){
            if(compareDate(choosenList->items[j], choosenList->items[j + 1]) > 0){
                Show* temp = choosenList->items[j];
                choosenList->items[j] = choosenList->items[j + 1];
                choosenList->items[j + 1] = temp;
            }
        }
    }
}
