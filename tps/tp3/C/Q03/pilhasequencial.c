...
Pilha* initPilha(int max){
    ...
    pilha->n = 0;
    pilha->max = max;
    pilha->elementos = malloc(sizeof(Show*));
    return pilha;
}

void freePilha(Pilha* pilha){
    if(pilha == NULL){
        printf("Erro - tentando desalocar pilha inexistente");
        return;
    }
    for(int i = 0; i < pilha->n; i++){
       free(pilha->elementos[i]);
    }
    free(pilha);
}

void inserePilha(Pilha* pilha, Show* show){
    if(pilha == NULL){
        printf("Erro - tentando inserir Show em uma pilha inexistente");
        return;
    }
    if(show == NULL){
        printf("Erro - tentando alocar na pilha um show inexistente");
        return;
    }
    pilha->elementos[pilha->n++] = show;
}

Show* removePilha(Pilha* pilha){
    if(pilha == NULL){
        printf("Erro - tentando remover Show de uma pilha inexistente");
        return;
    }
    if(pilha->n == 0){
        printf("Erro - tentando remover Show de uma pilha vazia");
        return;
    }

    return pilha->elementos[--pilha->n];
}

void operaRegistro(Pilha* choosenShows, MyList* showList, char** entrada){
    if(!entrada[0]) return;
    if(strcmp(entrada[0], "I") == 0 && entrada[1]){
        Show* inserir = findByShowId(showList, entrada[1]);
        inserePilha(choosenShows, inserir);
    }else if(strcmp(entrada[0], "R") == 0){
        Show* remover = removePilha(choosenShows);
        printf("(R) %s\n", remover->title);
    }
}

void printaPilha(Pilha* choosenShows){
    if(choosenShows == NULL){
        printf("Erro - Pilha inexistente no printa pilha");
        return;
    }
    if(choosenShows->n == 0){
        printf("Erro - Pilha vazia no printa pilha");
        return;
    }
    for(int i = choosenShows->n - 1; i > 0; i--){
        printf("[%d] ", i);
        printShow(choosenShows->elementos[i]);
    }
}
...
int main() {
    ...
    Pilha* choosenShows = initPilha(1000);
    for(int i = 0; i < idCount; i++) {
        Show* s = findByShowId(showList, idList[i]);
        if (s) {
            inserePilha(choosenShows, s);
        } else {
