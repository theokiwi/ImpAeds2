...
void inserirPilha(Pilha* pilha, Show* elemento) {
...
Show* removerPilha(Pilha* pilha) {
    if (pilha->topo == NULL) {
        printf("Erro - Pilha vazia no remover\n");
        return NULL;
    }
...
void operaRegistro(Pilha* choosenShows, MyList* showList, char** entrada){
    ...
    if(strcmp(entrada[0], "I") == 0 && entrada[1]){
        ...
        inserirPilha(choosenShows, inserir);
    }else if(strcmp(entrada[0], "R") == 0){
        Show* remover = removerPilha(choosenShows);
        printf("(R) %s\n", remover->title);
    }
}
...
int main() {
    ...
    Pilha* choosenShows = novaPilha(1000);
    for(int i = 0; i < idCount; i++) {
        Show* s = findByShowId(showList, idList[i]);
        if (s) {
            inserirPilha(choosenShows, s);
        } else {
