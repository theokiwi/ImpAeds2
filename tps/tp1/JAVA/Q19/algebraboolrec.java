...
public class algebraboolrec {
    //eu fiz o que era possivel recursivo
    ...
    public static int resolveEquacaoRec(String entrada) {
        if (tem(entrada, '(')) {
            int comecaParenteses = entrada.lastIndexOf('(');
            int terminaParenteses = entrada.indexOf(')', comecaParenteses);
            String pedaco = subString(entrada, comecaParenteses - 1, terminaParenteses + 1);
            String resolucao = resolveExp(pedaco);
            String expressao = subString(entrada, 0, comecaParenteses - 1) + resolucao + subString(entrada, terminaParenteses + 1, entrada.length());
            return resolveEquacaoRec(expressao);
        } else {
    ...
    public static void main(String[] args) {
        ...
        while(rc.hasNext()){
            ...
            if(!(compararStrings(entrada, "0"))){
                ...
                int resposta = resolveEquacaoRec(saida);
